...
export async function POST(request: NextRequest) {
  try {
    const { message, conversation_history, session_id, user_id, stream } =
      await request.json();
      ...
      try {
        ...
        if (stream) {
          const backendResponse = await fetch(
            `${FASTAPI_BACKEND_URL}/chat/stream`,
            {
              method: "POST",
              headers: {
          ...
          // Re-pipe the stream chunk by chunk to prevent Node.js buffering
        ...
        if (!backendResponse.ok) {
          ...
          throw new Error(
            errorData.detail || `Backend error: ${backendResponse.statusText}`,
          );
        }
    ...
    if (!process.env.GOOGLE_API_KEY) {
      return NextResponse.json(
        {
          error:
            "No backend configured. Please set either USE_FASTAPI_BACKEND=true with FastAPI running, or set GOOGLE_API_KEY in .env.local",
        },
        { status: 500 },
      );
    }
